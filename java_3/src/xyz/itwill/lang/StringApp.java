...
//StirngApp 클래스 : 문자열을 저장하고 문자열에 대한 다양한 기능을 메소드로 제공하는 클래스
//=> String 객체에 저장된 문자열에 대한 다양한 기능을 메소드로 제공
//=> String 객체는 문자열을 내부적으로 byte 배열로 처리하여 배열 요소에 문자를 하나씩 저장
public class StringApp {
	public static void main(String[] args) {
		//문자열을 " " 기호를 사용하여 표현하면 자동으로 메모리의 메소드영역(MethodArea)에 String 객체를 생성하여 String 객체에 문자열 저장
		...
		//new 연산자로 생성자를 호출하여 String 객체를 생성하면 메모리의 힙영역(HeapArea)에 새로운 String 객체를 생성하여 사용
		...
		if(str1.equals(str3)) {
			...
			System.out.println("str1 변수와 str3 변수에 저장된 String 객체의 메모리 문자열이 다릅니다.");
		}
		...
		if(str1.compareTo(str4)>0) {
			...
			System.out.println("str1 변수에 저장된 문자열이 str4에 저장된 문자열보다 같습니다.");
		}
		
		System.out.println("==============================================================");
		
		//String.getBytes() : String 객체에 저장된 문자열을 byte 배열로 변환하여 반환하는 메소드.
		//=> byte 배열의 요소에는 문자열을 구성하는 문자들이 코드값(정수값)으로 차례대로 저장된다.
		byte[] array=str1.getBytes();
		
		for(byte ch : array) {
			System.out.print(ch);	//정수값으로 나옴
			System.out.println((char)ch); 	//byte 배열의 요소값(정수값)을 문자로 형변환하여 출력
		}
		System.out.println();
		...
		//String.ChatAt(int index) : String 객체에 저장된 문자열에서 첨자(Index) 위치의 문자를 반환하는 메소드
		System.out.println("두번째 위치의 문자 = "+str5.charAt(1)); 	//첨자는 0부터 시작하니까 2번째 위치의 문자의 첨자는 1이다.
		...
		System.out.println("str6 = "+str6.toUpperCase());	//-> 문자 전부 대문자됨
		System.out.println("str6 = "+str6.toLowerCase());	//-> 문자 전부 소문자됨
		...
		//String.trim() : String 객체에 저장된 문자열에서 앞과 뒤에 존재하는 모든 공백을 제거하여 변환하는 메소드 (중간의 공백은 제거 못함. 앞과 뒤만)
...
//		String[] numArray=str9.split("\\*");
		...
		System.out.println("전화번호 앞부분 = "+numArray[0]);	//전화번호를 특정문자(-)로 분리해서 처리
		System.out.println("전화번호 중간부분 = "+numArray[1]);
		System.out.println("전화번호 뒷부분 = "+numArray[2]);
		
		System.out.println("==============================================================");

		//String.substring(int beginIndex, int endIndex) : String 객체에 제정된 문자열에서 시작첨자(문자포함) 종료첨자(문자 미포함)까지의 문자열을 분리하여 반환하는 메소드
		System.out.println("전화번호 앞부분 = "+str9.substring(0, 3));	//전화번호를 특정문자(-)로 분리해서 처리
...
//		String numString = String.valueOf(100);
		...
		System.out.println("mumString = "+numString);
