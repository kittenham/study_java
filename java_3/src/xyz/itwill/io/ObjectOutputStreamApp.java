...
public class ObjectOutputStreamApp {
	public static void main(String[] args) throws IOException {
		...
		ObjectOutputStream out = new ObjectOutputStream(new FileOutputStream("c:/data/data.txt"));
			//ObjectOutputStream 클래스 : OutputStream 객체를 전달받아 Java 객체를 원시데이터로 전달하는
			//출력스트림을 생성하기 위한 클래스 (값을 하나씩 원시데이터로 전달하는 것이 아니라 "객체"를 원시데이터로 전달)
