...
//=> 서버와 클라이언트가 소켓을 이용하여 1:1로 연결되어 데이터를 송수신하는 네트워크 프로그램

//서버(Server) : 외부의 컴퓨터(클라이언트)가 접속할 수 있는 "환경"과 서비스를 제공하기 위한 컴퓨터
	...
	public static void main(String[] args) {
		for(int i=2000;i<=9000;i+=1000) {
			try {
				...
				// => ServerSocket 클래스의 ServerSocket(int port) 생성자를 이용하여 매개변수로 전달받은 포트번호를 활성화하여
				//ServerSocket 객체 생성
				//ServerSocket 객체를 생성하여 클라이언트가 접속할 수 있는 환경 제공
				//=> 매개변수로 전달받은 포트번호를 이미 다른 네트워크 프로그램에 사용중인 경우 IOException 발생
				ServerSocket serverSocket = new ServerSocket(i);
						
				//이때, 방화벽허용을 해야 네트워크 연결이 제대로 일어난다.
				
				System.out.println("[메세지]\"+i+\"번 포트는 네트워크 프로그램에서 사용가능합니다..");
