...
public class DecimalFormatApp {
	...
	public static void main(String[] args) {
		...
		long money = 100_000_000;
		System.out.println("금액 = "+money+"원");
		
		//여기까지 출력 :[ 금액 = 100000000원 ]
		//더 보기 좋게 만들기
		
		//DecimalFormat 클래스로 객체를 생서하기 위한 매개변수에 숫자값에 대한 패턴정보를 문자열로 전달받아
		//객체로 생성하는 생성자를 사용
		//=> 패턴문자 : #(숫자-공백), 0(숫자-0), 콤마(,), 소숫점(.), 달러($)
		
		DecimalFormat decimalFormat = new DecimalFormat("###,###,##0");	//100000000을 중간에 콤마를 넣어 보기 편하게 만드는 방법
		
		//DecimalFormat.format(long number) : 숫자값을 전달받아 DecimalFormat 객체에 저장된 패턴정보를
		//사용하여 문자열로 변환하여 반환하는 메소드
		System.out.println("금액 = "+decimalFormat.format(money)+"원");
		
		//DecimalFormat.getInstance() : 기본 패턴정보가 저장된 DecimalFormat 객체를 반환하는 메소드
		//=> 기본 패턴정보 : 3자리마다 콤마(,)를 찍어줌.
