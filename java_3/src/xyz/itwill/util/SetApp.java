...
//콜렉션 클래스(Collection Class) : 자료구조(Data Structure)를 사용하여 *다수의 객체*를 효율적으로 관리(추가,삭제,검색)하기 위한 
//기능을 제공하는 데이터 수집 클래스를 통칭
//@값을 여러개 저장하고 싶을때 사용하는 건 배열./ 객체를 여러개 저장해 관리하기 위한건 콜렉션 클래스.

//=> Set 인터페이스, List 인터페이스, Map 인터페이스 중 하나를 상속받아 작성된 클래스
//=> 이때 Set, List 인터페이스는 Collection 인터페이스를 상속받는다.(그래서 Set과 List는 거의 비슷함)

//Set 인터페이스를 상속받은 콜렉션 클래스 - HashSet, TreeSet 등 (Oracle 홈페이지 들어가면 자식클래스 종류 많이 나옴)
//=> *객체의 저장 순서가 없으며*(단점->내가 원하는 값을 뽑아낼수가없다.), 
//*메모리 주소(hashCode)와 객체에 저장된 값이 동일한 객체는 중복 저장되지 않는다*.(장점=똑같은 값을 같는 객체가 없다는 말)
...
//		HashSet<String> set = new HashSet<String>();
		...
		//요소(Element) : Collection 클래스의 객체에 제너릭타입으로 전달되어 저장되는 객체 (=객체의 객체)
		set.add("홍길동");
		set.add("임꺽정");
		set.add("전우치");
//		set.add(12.34);	//Set 객체에 String객체를 제외한 객체를 요소로 저장할 경우 에러 발생(12.34는 Double이니까)
		...
		//Iterator 객체 : 콜렉션 클래스의 객체에 저장된 요소를 반복처리하기 위한 기능을 제공하는 객체
		//=> "반복 지시자(=반복자)" : 커서(Cursor)를 사용하여 콜렉션 객체의 요소를 반복처리한다.
		//커서(Cursor) : 값을 읽어들이는 위치 제공. 
		Iterator<String> iterator = set.iterator();
		
		//*일괄처리 꼭 해줘야함.(순서가 없어서 요소 반환메소드X. 따라서, 한번에 처리해야함)
		//하는 방법은 2가지. Iterator 객체를 직접사용 / 향상된 for문 사용
