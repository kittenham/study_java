...
public class BorderLayoutApp extends Frame{
	...
	public BorderLayoutApp(String title) {
		...
		//=> Frame의 기본 배치관리자는 [BorderLayout]이므로 배치관리자 생략 가능
		...
		//Container.add(Component c, Object constraints) : 컴퍼넌트를 컨테이너의 원하는 위치에 배치하는 메소드 
		//- 배치관리자가 [BorderLayout]인 경우에만 사용
		//=> 컴퍼넌트가 배치되는 위치는 BorderLayout 클래스의 상수(Constant)사용
		//=> 컨테이너의 *중앙(Center)에는 반드시 컴퍼넌트를 배치해야 되지만, 동서남북은 생략 가능
