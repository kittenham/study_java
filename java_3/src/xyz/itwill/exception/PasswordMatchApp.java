...
/* 방법1		
		...
		if(number==password) {
			...
			System.out.println("[결과] 입력된 비밀번호가 틀렸습니다.");
		}
		scanner.close();
*/
...
//방법2		
		...
		try {
			...
			if(number!=password) {	//예외를 일부러 발생시켜서 예외 객체를 던져줄 수 있다. 원래는 예외객체는 JVM이 만드는
//				throw new Exception();	// 인위적으로 예외객체를 생성하여 예외를 발생시키는 방법 => 여기서는 Exception이라는 에러 발생시킴
										// 형식)  throw new 예외클래스(String message);
										// 인위적으로 예외를 만든 것이기 때문에 내가 예외를 따로 생성해줘야함. class : PasswordMismatchException 참고
										// PasswordMismatchException 클래스는 개발자가 직접 생성한 예외클래스로 예외처리를 하지 않을 경우 에러 발생 - 일반예외
