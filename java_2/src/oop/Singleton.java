...
//- 데이터 처리 클래스는 싱글톤 클래스로 만드는 것이 효율적
public class Singleton {
	
	//클래스의 객체(메모리주소)를 저장하기 위한 필드를 선언한다.
	//static 제한자를 사용하여 필드를 선언한다.
	//시스템 필드 : 클래스 내부에서만 사용하기 위한 필드로서, Getter 메소드와 Setter메소드를 선언하지 않는다.(외부에서 사용하지 않기위해)
	//=> 일반적인 필드와 구분하기 위해 필드명을 _로 시작되도록 작성하는 것을 권장
	private static Singleton _instance;
	
	//public을 private로 바꿈 -> ***생성자를 은닉화 선언 한 것.*** : 클래스 외부에서 생성자에 접근하지 못하도록 설정
	//=> 클래스 외부에서 객체생성이 불가능하도록. + 상속불가 (상속성을 위반했음)
	private Singleton() {
