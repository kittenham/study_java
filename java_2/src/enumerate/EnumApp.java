...
public class EnumApp {
	public static void main(String[] args) {
		//열거형에 선언된 상수필드값 출력 -> 상수값(0~3값)이 아닌 상수필드의 이름이 제공되어 출력된다.
		...
		//열거형에 선언된 상수를 저장하기 위해서는 반드시 열거형(참조형)을 이용하여 변수를 선언한다.
		//=> 상수필드가 선언된 열거형을 하나의 자료형으로 사용 가능하다.
//		int choice=EnumOne.INSERT;	//=> 에러발생
		//choice 변수에는 EnumOne 자료형에 선언된 상수만 저장 가능하다.
		EnumOne choice = EnumOne.INSERT;	//=>에러X
		System.out.println("choice = "+choice);
		
		//나열형으로 선언된 변수에 비교값은 같은 나열형에 선언된 상수만 사용하여 비교가능
		switch(choice) {
		case INSERT:
