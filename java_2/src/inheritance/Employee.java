...
public abstract class Employee {
	...
	//상수필드(Constant Field) : 프로그램에서 값(리터럴) 대신 사용하기 위한 의미있는 단어로 제공되는 필드값(대표값)- 상수라고 부르기도함
	//형식)  public static final 자료형 변수명 = 초기값;
	//=> 상수명은 대문자로 작성하여 스네이크 표기법(대문자, _사용)을 이용하여 작성하는 것을 권장
	//클래스 안에 하나만 만들어짐
...
*/
	...
	//final 쓰는 이유 : 자식클래스에서 메소드를 오버라이드 선언하면 비정상적인(잘못된) 결과값이 반환될 수 있기 때문. 자식클래스의 메소드를 쓰는 것이 아닌 부모클래스의 메소드를 사용해야하는 경우.
...
//		return (int)(computePay()*1.5);		//그냥 1.5하면 이게 인센트브율인지 잘 모를 수 있음.
		return (int)(computePay()*INCENTIVE_RATE);		//위랑 똑같음(상수필드 사용한 것) -> 1.5를 상수필드를 이용하여 인센티브인상률이라는 것을 명시한것.(보고 해석하기 쉽게!)
