	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < lotto.length; i++) { // 새로운 난수를 저장한 배열 요소를 표현할 첨자의 반복
			while (true) {
				...
				boolean result = false; // 중복상태를 저장하기 위한 변수 - [false : 미중복, true : 중복] 약속
				...
				if (!result)
					break; // =false가 아니라면 = true라면

			} // 새로운 난수가 기존 난수값과 중복되지 않는 경우, while문을 종료. //=> 이렇게 하면 절대 중복값이 나오지 않음.
		}
		...
		for (int i = 0; i < lotto.length - 1; i++) { // (비교하기 위한 배열 요소의 첨자를 표현하는 반복문 : 0~끝-1 (마지막은 비교할필요X))
			for (int j = i + 1; j < lotto.length; j++) { // 피비교 요소의 첨자를 표현하는 반복문 : 비교+1 ~ 끝
				...
				if (lotto[i] > lotto[j]) { // => 요소에 저장된 값을 서로 바꾸어 저장 = Swap 알고리즘
					int temp = lotto[i];
					lotto[i] = lotto[j];
					lotto[j] = temp;
				}
